...
class AkunadminController extends Controller
{
    ...
    {
        $data['title'] = 'Akun Admin Toko';
        $data['rs_admin'] = User::with('users_data')->where('role_id', 'R0004')->paginate('10');
        // dd($data);
        return view('admin.akun_admin.index', $data);
    }
    ...
    {
        $data['title'] = 'Tambah Akun Admin Toko';
        return view('admin.akun_admin.add', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        //redirect
        return redirect()->route('addAkunAdmin')->with('success', 'Data berhasil disimpan');
    }
    ...
    {
        $data['title'] = 'Ubah Akun Admin Toko';
        $detail = User::with('users_data')->where('user_id', $id)->first();
        $data['detail'] = $detail;
        // dd($detail);
        return view('admin.akun_admin.edit', $data);
    }
    ...
    public function update(Request $request, string $user_id)
    {
        ...
        if (empty($detail)) {
            return redirect()->route('akunAdmin')->with('error', 'Data tidak ditemukan');
        }
        $request->validate([
        ...
        if ($detail->save()) {
            ...
            if (!empty($detailUser)) {
                ...
                //
                if ($detailUser->save()) {
                    return redirect()->route('editAkunAdmin', [$user_id])->with('success', 'Data berhasil diupdate');
                }
            }
        } else {
            return redirect()->route('akunAdmin')->with('error', 'Gagal update date');
        }
    }
    ...
    public function destroy(string $user_id)
    {
        ...
        if ($detail->delete()) {
            return redirect()->route('akunAdmin')->with('success', 'Data berhasil dihapus');
        } else {
            return redirect()->route('akunAdmin')->with('error', 'Data gagal dihapus');
        }
    }
}
